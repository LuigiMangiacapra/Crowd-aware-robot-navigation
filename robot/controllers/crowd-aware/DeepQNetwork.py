...
class DQN(nn.Module):
    ...
    def __init__(self,
        ...
        # Primo layer: input dim (196) -> hidden dim (64)
        self.fc1 = nn.Linear(input_dim, hidden_dim)
        self.activation = nn.LeakyReLU()

        self.fc2 = nn.Linear(hidden_dim, hidden_dim)
        self.fc3 = nn.Linear(hidden_dim, n_robot_actions * n_human_actions)

        self.n_robot_actions = n_robot_actions
        self.n_human_actions = n_human_actions

    def forward(self, x):
        # print(f"[DQN forward] Input shape: {x.shape}")
        x = self.activation(self.fc1(x))
        x = self.activation(self.fc2(x))
        x = self.fc3(x)
        x = x.view(-1, self.n_robot_actions, self.n_human_actions)
        # print(f"[DQN forward] Output shape: {x.shape}")  # atteso: [batch, 5, 4]
        return x
